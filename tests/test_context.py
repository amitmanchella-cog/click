...
def test_parameter_source_during_convert_and_callback(
    ...
    """The source is available while type conversion and the callback run."""
    seen = {}
    ...
    kwargs = {"default": 1, "type": Source(), "callback": callback}
    kwargs.update(option_args)

    @click.command()
    @click.pass_context
    @click.option("-o", "--option", **kwargs)
    def cli(ctx, option):
        return ctx.get_parameter_source("option")
...
def test_parameter_source_eager_callback(runner):
    seen = {}
    ...
    assert seen["source"] is ParameterSource.DEFAULT
    runner.invoke(cli, ["--debug"], standalone_mode=False)
    assert seen["source"] is ParameterSource.COMMANDLINE
